...
public class GridPathfinding : MonoBehaviour
{
    ...
    public void UpdateGrid(float xSize, float ySize, Vector2 updatePosition)
    {
        Vector2 worldBottomLeft = (Vector2)transform.position - Vector2.right * gridWorldSize.x / 2 - Vector2.up * gridWorldSize.y / 2;
        Debug.Log(worldBottomLeft);

        int xTrack = (int)Mathf.Floor((updatePosition.x - (xSize/2) - worldBottomLeft.x) / (nodeDiameter));
        int xTrackEnd = (int)Mathf.Floor((updatePosition.x + (xSize/2) - worldBottomLeft.x) / (nodeDiameter));
        int yTrack = (int)Mathf.Floor((updatePosition.y - (ySize/2) - worldBottomLeft.y) / (nodeDiameter));
        int yTrackEnd = (int)Mathf.Floor((updatePosition.y + (ySize/2) - worldBottomLeft.y) / (nodeDiameter));

        for (int x = xTrack; x < xTrackEnd; x++)
        {
            for (int y = yTrack; y < yTrackEnd; y++) 
            {
                Vector2 worldPoint = worldBottomLeft + Vector2.right * (x * nodeDiameter + nodeRadius) + Vector2.up * (y * nodeDiameter + nodeRadius);
                bool walkable = (Physics2D.OverlapCircle(worldPoint, nodeRadius, unwalkableMask) == null); // if no collider2D is returned by overlap circle, then this node is walkable
